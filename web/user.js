...
const getLogTimestamp = (log) => log.createdAtMs ?? (log.createdAt ? new Date(`${log.createdAt}Z`).getTime() : null);
...
const isHubEvent = (log) => log.type === 'hub_raw' || log.type === 'hub' || /^Событие хаба/.test(log.text ?? '');

const renderLogs = (logs) => {
  const filtered = logs.filter((log) => {
    if (isHubEvent(log)) return false;
  ...
  filtered.forEach((log) => {
    ...
    const dateLabel = formatLogDate(timestamp);
