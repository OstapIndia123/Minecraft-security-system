...
const renderDeviceDetails = (device) => {
  ...
  if (device.type === 'zone') {
    return `
      ...
      <input type="number" name="delaySeconds" value="${device.config?.delaySeconds ?? ''}" min="1" placeholder="Задержка (сек)" />
  ...
  if (editForm) {
    ...
    const zoneTypeSelect = editForm.querySelector('select[name="zoneType"]');
    const delayInput = editForm.querySelector('input[name="delaySeconds"]');
    if (zoneTypeSelect && delayInput) {
      const updateDelayField = () => {
        ...
        if (needsDelay && !delayInput.value) {
          delayInput.value = device.config?.delaySeconds ?? 30;
        }
      };
      zoneTypeSelect.addEventListener('change', updateDelayField);
      updateDelayField();
    }
  }
};
  ...
  chip.addEventListener('click', async () => {
    ...
    try {
      ...
      if (action === 'arm') {
        ...
        if (updated?.pendingArm) {
          Object.assign(space, updated.space);
          showToast('Начало снятия.');
        } else {
...
if (deviceType) {
  const updateDeviceFields = () => {
    ...
    const zoneTypeSelect = zoneFields?.querySelector('select[name="zoneType"]');
    const delayInput = zoneFields?.querySelector('input[name="delaySeconds"]');
    if (zoneTypeSelect && delayInput) {
      const needsDelay = ['delayed', 'pass'].includes(zoneTypeSelect.value);
      delayInput.classList.toggle('hidden', !needsDelay);
      delayInput.disabled = !needsDelay || !isZone;
      delayInput.required = needsDelay && isZone;
      if (needsDelay && !delayInput.value) {
        delayInput.value = '30';
      }
    }
  ...
  const zoneTypeSelect = zoneFields?.querySelector('select[name="zoneType"]');
  if (zoneTypeSelect) {
    zoneTypeSelect.addEventListener('change', updateDeviceFields);
  }
}
...
if (readKey && deviceForm) {
  readKey.addEventListener('click', async () => {
    const space = spaces.find((item) => item.id === state.selectedSpaceId);
    if (!space) return;
    try {
      showLoading();
      ...
      console.error(error);
      showToast('Нет доступных сканов ключа.');
    } finally {
      hideLoading();
    }
  });
