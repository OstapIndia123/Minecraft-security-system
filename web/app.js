...
const translations = {
  ru: {
    ...
    'engineer.installers.leave': 'Покинуть пространство',
    'engineer.members.self': 'вы',
  ...
  'en-US': {
    ...
    'engineer.installers.leave': 'Leave space',
    'engineer.members.self': 'you',
...
const syncProfileSettings = async () => {
  try {
    ...
    state.userId = result.user.id ?? state.userId;
    saveProfileSettings();
...
const renderMembers = (members) => {
  ...
  const currentMember = members.find((member) => member.id === state.userId);
  state.memberRole = currentMember?.role ?? null;
  if (leaveSpaceBtn) {
    leaveSpaceBtn.classList.toggle('hidden', !currentMember);
  }
  ...
  if (usersList) {
    ...
    if (!users.length) {
      ...
      users.forEach((member) => {
        ...
        const baseLabel = member.minecraft_nickname ? `${member.minecraft_nickname} (${member.email})` : member.email;
        const label = member.id === state.userId ? `${baseLabel} · ${t('engineer.members.self')}` : baseLabel;
        card.innerHTML = `
          <div>
            <div class="member-card__title">${label}</div>
            <div class="member-card__meta">ID: ${member.id}</div>
          </div>
          ${member.id === state.userId ? '' : `
...
if (leaveSpaceBtn) {
  leaveSpaceBtn.addEventListener('click', async () => {
    if (!state.selectedSpaceId || !state.userId) return;
    try {
      if (!await confirmAction('Покинуть пространство?', t('engineer.installers.leave'))) return;
      showLoading();
      await removeMember(state.userId);
      showToast('Вы покинули пространство.');
      await loadSpaces();
      renderAll();
    } catch (error) {
      if (error.message === 'last_installer') {
        showToast('Нельзя покинуть пространство: вы единственный инженер.');
      } else {
        handleApiError(error, 'Не удалось покинуть пространство.');
      }
    } finally {
      hideLoading();
    }
