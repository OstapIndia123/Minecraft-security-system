...
const translations = {
  ru: {
    ...
    'auth.errors.nickname_cooldown': 'Сменить ник можно раз в 3 дня.',
  ...
  'en-US': {
    ...
    'auth.errors.nickname_cooldown': 'You can change your nickname once every 3 days.',
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (payload?.error === 'nickname_cooldown') {
          showError(translations[language]?.['auth.errors.nickname_cooldown'] ?? 'Nickname can be changed every 3 days.');
          return;
        }
