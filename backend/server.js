...
const startLightBlinking = async (spaceId, hubId) => {
  if (!hubId || lightBlinkTimers.has(spaceId)) return;
  ...
  lightBlinkTimers.set(spaceId, timer);
};

const stopLightBlinking = async (spaceId, hubId) => {
  const timer = lightBlinkTimers.get(spaceId);
  if (timer) {
    clearInterval(timer);
    lightBlinkTimers.delete(spaceId);
  }
  if (hubId) {
    const statusRow = await query('SELECT status FROM spaces WHERE id = $1', [spaceId]);
    const status = statusRow.rows[0]?.status ?? 'disarmed';
    await applyLightOutputs(spaceId, hubId, status);
  }
};
...
const getDelaySeconds = (zones) => zones.reduce((max, zone) => {
  const seconds = Number(zone.config?.delaySeconds ?? 30);
  if (Number.isNaN(seconds)) return max;
  return Math.max(max, seconds);
}, 0);

const getDelayZones = async (spaceId) => {
  const result = await query('SELECT id, name, status, config FROM devices WHERE space_id = $1 AND type = $2', [
    spaceId,
    'zone',
  ]);
  return result.rows.filter((zone) => {
    ...
    return !bypass && zone.status !== 'Норма' && ['delayed', 'pass'].includes(zoneType);
  });
};

const attemptArm = async (spaceId, who, logMessage) => {
  ...
  if (hasBypass) {
    await appendLog(spaceId, 'Неудачная постановка (обход зоны активен)', who, 'security');
    return { blocked: 'bypass' };
  }

  const violations = zones.rows.filter((zone) => zone.status !== 'Норма' && !zone.config?.bypass);
  const delayZones = violations.filter((zone) => ['delayed', 'pass'].includes(zone.config?.zoneType ?? 'instant'));
  const blockingZones = violations.filter((zone) => !['delayed', 'pass'].includes(zone.config?.zoneType ?? 'instant'));

  if (blockingZones.length) {
    await appendLog(spaceId, 'Неудачная постановка (зоны не в норме)', who, 'security');
    return { blocked: 'zone_state' };
  }

  if (delayZones.length) {
    const spaceRow = await query('SELECT hub_id FROM spaces WHERE id = $1', [spaceId]);
    const delaySeconds = getDelaySeconds(delayZones);
    await startExitDelay(spaceId, spaceRow.rows[0]?.hub_id, who, delaySeconds);
    return { pending: true };
  }

  return { space: await updateStatus(spaceId, 'armed', who, logMessage) };
};

const startExitDelay = async (spaceId, hubId, who, delaySeconds) => {
  if (exitDelayTimers.has(spaceId)) return;
  await appendLog(spaceId, 'Начало снятия', who, 'security');
  await startLightBlinking(spaceId, hubId);
  const timer = setTimeout(async () => {
    exitDelayTimers.delete(spaceId);
    const zones = await getDelayZones(spaceId);
    if (zones.length) {
      await appendLog(spaceId, 'Неудачная попытка постановки под охрану', who, 'security');
      await stopLightBlinking(spaceId, hubId);
      return;
    }
    await updateStatus(spaceId, 'armed', who);
    await stopLightBlinking(spaceId, hubId);
  }, Math.max(delaySeconds, 1) * 1000);
  exitDelayTimers.set(spaceId, timer);
};

const stopExitDelay = async (spaceId, hubId) => {
  const timer = exitDelayTimers.get(spaceId);
  if (timer) {
    clearTimeout(timer);
    exitDelayTimers.delete(spaceId);
    if (hubId) {
...
const startEntryDelay = async (spaceId, hubId, who, delaySeconds) => {
  if (entryDelayTimers.has(spaceId)) return;
  await appendLog(spaceId, 'Начало снятия', who, 'security');
  await startLightBlinking(spaceId, hubId);
  const timer = setTimeout(async () => {
    ...
    if (status === 'armed') {
      ...
      await startSirenTimers(spaceId, hubId);
...
const stopEntryDelay = async (spaceId, hubId) => {
...
app.get('/api/spaces/:id/last-key-scan', async (req, res) => {
  const scan = lastKeyScans.get(req.params.id);
  if (!scan) return res.status(404).json({ error: 'no_key_scan' });
  lastKeyScans.delete(req.params.id);
  res.json(scan);
});
...
const deviceConfigFromPayload = (payload) => {
  ...
  if (payload.type === 'zone') {
    const zoneType = payload.zoneType ?? 'instant';
    const delaySeconds = payload.delaySeconds;
    return {
      zoneType,
      bypass: payload.bypass === 'true',
      silent: payload.silent === 'true',
      delaySeconds: delaySeconds === undefined || delaySeconds === ''
        ? (['delayed', 'pass'].includes(zoneType) ? 30 : null)
        : Number(delaySeconds),
...
const updateStatus = async (spaceId, status, who, logMessage) => {
  ...
  if (status !== 'armed') {
    ...
    await stopEntryDelay(spaceId, space.hubId);
    await stopExitDelay(spaceId, space.hubId);
  }
...
const handleReaderScan = async ({ readerId, payload, ts }) => {
  ...
  lastKeyScans.set(spaceId, { readerId, keyName });
...
app.post('/api/spaces/:id/arm', async (req, res) => {
  const spaceRow = await query('SELECT id FROM spaces WHERE id = $1', [req.params.id]);
  if (!spaceRow.rows.length) return res.status(404).json({ error: 'space_not_found' });
  const result = await attemptArm(req.params.id, 'UI');
  if (result.blocked) return res.status(409).json({ error: result.blocked });
  if (result.pending) {
    const current = await query('SELECT * FROM spaces WHERE id = $1', [req.params.id]);
    const space = mapSpace(current.rows[0]);
    space.devices = await loadDevices(space.id, space.hubId, space.hubOnline);
    return res.json({ pendingArm: true, space });
  }
  if (!result.space) return res.status(404).json({ error: 'space_not_found' });
  return res.json(result.space);
});
...
app.post('/api/hub/events', requireWebhookToken, async (req, res) => {
  ...
  if (type === 'PORT_IN' && payload?.side && payload?.level !== undefined) {
    ...
    for (const zone of zones.rows) {
      ...
      const entryDelayActive = entryDelayTimers.has(spaceId);
      const exitDelayActive = exitDelayTimers.has(spaceId);

      if (shouldCheck && !bypass && !isNormal) {
        if (status === 'armed' && zoneType === 'delayed' && !entryDelayActive) {
          const delaySeconds = Number(config.delaySeconds ?? 30);
          await startEntryDelay(spaceId, spaceRow.rows[0]?.hub_id, 'Zone', delaySeconds);
        } else if (status === 'armed' && zoneType === 'pass' && !entryDelayActive) {
          ...
          await query('UPDATE spaces SET issues = true WHERE id = $1', [spaceId]);
        } else if (!entryDelayActive && !exitDelayActive) {
          await appendLog(spaceId, `Тревога шлейфа: ${zone.name}`, 'Zone', 'alarm');
          spaceAlarmState.set(spaceId, true);
          if (!silent) {
            await startSirenTimers(spaceId, spaceRow.rows[0]?.hub_id);
